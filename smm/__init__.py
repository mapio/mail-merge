...
TOKEN_GENERATION_AUTH = environ['TOKEN_GENERATION_AUTH']
...
def token():
    auth = request.headers.get('X-TOKEN-GENERATION-AUTH')
    name = request.form.get('name', None)
    mail = request.form.get('mail', None)
    if auth is None or auth != TOKEN_GENERATION_AUTH: abort(401)
    return Response(
      'Token for "{} <{}>":\n\n{}{}\n'.format(
        name,
        mail,
...
def index(token = None):
    ...
    if status != 'OK' or request.method == 'GET':
      ...
      if not subject: subject = '[scythe-mail-merge]: Mail merge from {}'.format(info['name'])
